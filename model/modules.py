...
class ReferenceEncoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # print("input shape is {}".format(out.shape))
        out = self.conv(out)
...
class WSTEncoder(nn.Module):
    ...
    def __init__(self):
        super(WSTEncoder, self).__init__()
        self.filter_size = [1] + hp.wst_ref_filters
        self.dropout = hp.encoder_dropout
        self.conv = nn.Sequential(
            OrderedDict(
                [
    ...
    def forward(self, inputs):
        ...
        out = self.conv(out)
        out = out.view(out.shape[0], out.shape[1], -1).contiguous()
        #out = out.contiguous().transpose(2, 3)
        #self.gru.flatten_parameters()
        #memory, out = self.gru(out)
        #B * T * dim -> B * T * dim
        return out
...
class WSTAttention(nn.Module):
    def __init__(self):

        super().__init__()
        #self.input_size = hp.ref_gru_hidden
        self.input_size = hp.wst_ref_filters[-1]*2
        self.output_size = hp.wst_size
        self.n_token = hp.n_style_token
        self.n_head = hp.n_style_attn_head
        self.token_size = self.output_size // self.n_head
    ...
    def forward(self, inputs, token_id=None, get_score=False):
        ...
        word_num = style_emb.shape[1] 
        style_emb = style_emb.contiguous().view(self.n_head, bs, word_num, self.token_size)
        #print("SHAPE OF style_emb:{}".format(style_emb.shape))
